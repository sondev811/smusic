...
function MenuPlaylist(props) {
  ...
  const onPlay = async () => {
    ...
    if (!youtubeId || !user || !user.userInfo || !user.userInfo.queueListId) return;
    const findIndex = queue && queue.list && queue.list.length && 
      queue.list.findIndex(item => item.youtubeId === youtubeId);
    let res = queue;
    if (findIndex < 0 || queue.playlistId !== user.userInfo.queueListId) {
      const response = await musicService.insertSongPlaylist(user.userInfo.queueListId, youtubeId);
      if (response && response.status && response.result && response.result.playlist) {
        res = response.result.playlist
      }
    }
    playNow(user.userInfo.queueListId, res);
  }
  return (
